      ...
      map: [
        ...
        ...networks.map((network): V1beta1NetworkMapSpecMap => {
          ...
          const targetNAD = targetNADs.find(
            (nad) => nad.metadata.name === name && nad.metadata.namespace === namespace,
          );
...
export const getInitialStorageMap = ({
  ...
  const storageClasses = Array.from(
    new Set(
      pvcs
        .filter((pvc) => vmPVCs.includes(pvc.metadata.name))
  ...
  return {
    ...
    spec: {
      map: storageClasses.map((storageClass): V1beta1StorageMapSpecMap => {
        return {
          destination: {
            storageClass: targetStorageClasses.find((sc) => sc.metadata.name === storageClass)
              ?.metadata?.name,
          },
          source: {
            id: sourceStorageClasses.find((sc) => sc.metadata.name === storageClass)?.metadata?.uid,
            name: storageClass,
          },
        };
      }),
