...
/**
 * Converted to TS from: https://github.com/qemu/qemu/blob/b69801dd6b1eb4d107f7c2f643adf0a4e3ec9124/pc-bios/keymaps/no
 * cat input | sed -E "/^[^#]/s/\s+/', '/g" | sed -E "/^[^#]/s/^(.+)$/['\1'],/" | sed -E "/^[^#]/s/('(0x\w+)')/\2/"| sed -E "s/^#/\/\//" > output
 */

//TODO handle deprecated keysyms labelled with Unicode codepoint (number)
type Label = number | string;

const keymap: (
  | [Label, number, KeyModifier, KeyModifier]
  | [Label, number, KeyModifier]
  | [Label, number]
)[] = [
  //
