/**
 * Converted to TypeScript from: xorg-server-1.12.2/hw/xquartz/keysym2ucs.c
 * Source: https://www.x.org/releases/X11R7.7/src/xserver/xorg-server-1.12.2.tar.gz
...
export const keysym2ucs = (keysym: number): number => {
  let min = 0;
  let max = keysymtab.length - 1;
  let mid = 0;
...
const reverse_compare = (a: CodePair, b: CodePair) => a[1] - b[1];

const reverse_keysymtab = [...keysymtab].sort(reverse_compare);

export const ucs2keysym = (ucs: number): number => {
  let min = 0;
  let max = reverse_keysymtab.length - 1;
  let mid = 0;
  ...
  while (max >= min) {
    ...
    if (reverse_keysymtab[mid][1] < ucs) {
      ...
      /* found it */
      return reverse_keysymtab[mid][0];
    }
  }
