...
const useQuickStarts = (filterDisabledQuickStarts = true): WatchK8sResult<QuickStart[]> => {
  ...
  const watchResult = useK8sWatchResource<QuickStart[]>({
    groupVersionKind: getGroupVersionKindForModel(QuickStartModel),
    isList: true,
  });
  const quickStarts = watchResult[0];
  const quickStartsLoaded = watchResult[1];
  const quickStartsError = watchResult[2] as unknown;
