 ...
 */
export const getNetworkInterface = (vm: V1VirtualMachine, interfaceName: string): V1Interface =>
  getInterfaces(vm)?.find((iface) => iface?.name === interfaceName);
 ...
 */
export const getNetworkInterfaceState = (vm: V1VirtualMachine, interfaceName: string): string =>
  getNetworkInterface(vm, interfaceName)?.state;
