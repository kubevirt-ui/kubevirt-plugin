...
describe('CD-ROM selectors', () => {
  ...
  describe('getCDROMStatus', () => {
    ...
    describe('edge cases', () => {
      ...
      it('should handle VM with undefined disks from getDisks', () => {
        const vm = createVM('test-vm');

        mockIsRunning.mockReturnValue(false);
        mockGetDisks.mockReturnValue(undefined);
        mockGetVolumes.mockReturnValue([]);
      ...
      it('should handle different source types (dataVolume, PVC, containerDisk)', () => {
        const cdrom1 = createDisk({ cdrom: { bus: 'sata' }, disk: undefined, name: 'cdrom-dv' });
        const cdrom2 = createDisk({ cdrom: { bus: 'sata' }, disk: undefined, name: 'cdrom-pvc' });
        const cdrom3 = createDisk({
          cdrom: { bus: 'sata' },
          disk: undefined,
          name: 'cdrom-container',
        });

        const volumeDV = createVolume({ dataVolume: { name: 'my-datavolume' }, name: 'cdrom-dv' });
        const volumePVC = createVolume({
          name: 'cdrom-pvc',
          persistentVolumeClaim: { claimName: 'my-pvc' },
        });
        const volumeContainer = createVolume({
          containerDisk: { image: 'registry.io/iso:latest' },
          name: 'cdrom-container',
        });
        ...
        mockIsRunning.mockReturnValue(false);
        mockGetDisks.mockReturnValue([cdrom1, cdrom2, cdrom3]);
        mockGetVolumes.mockReturnValue([volumeDV, volumePVC, volumeContainer]);

        const result = getCDROMStatus(vm);

        expect(result).toHaveLength(3);
        expect(result[0].sourceName).toBe('my-datavolume');
        expect(result[1].sourceName).toBe('my-pvc');
        expect(result[2].sourceName).toBe('registry.io/iso:latest');
      });

      it('should handle running VM with multiple CD-ROMs - mixed eject/mount capabilities', () => {
        const cdromMounted = createDisk({
          cdrom: { bus: 'sata' },
          disk: undefined,
          name: 'cdrom-mounted',
        });
        const cdromUnmounted = createDisk({
          cdrom: { bus: 'sata' },
          disk: undefined,
          name: 'cdrom-unmounted',
        });

        const volumeMounted = createVolume({
          dataVolume: { name: 'iso-dv' },
          name: 'cdrom-mounted',
        });
        const volumeUnmounted = createVolume({ name: 'cdrom-unmounted' }); // Empty, no source

        const vmi = createVMI([cdromMounted, cdromUnmounted], [volumeMounted, volumeUnmounted]);
        const vm = createVM('test-vm');
        ...
        // Mounted CD-ROM on running VM: can eject, cannot mount
        expect(result[0]).toMatchObject({
          canDelete: false,
          canEject: true,
          canMount: false,
        ...
        // Unmounted CD-ROM on running VM: cannot eject, can mount
        expect(result[1]).toMatchObject({
          canDelete: false,
          canEject: false,
          canMount: true,
      ...
      it('should include disk and volume objects in the result', () => {
        const cdromDisk = createDisk({
          cdrom: { bus: 'sata' },
          disk: undefined,
          name: 'cdrom-disk',
        ...
        expect(result[0].disk).toEqual(cdromDisk);
        expect(result[0].volume).toEqual(volume);
      });
    });
  });
});
