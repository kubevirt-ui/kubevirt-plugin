...
export const getNamespaceOptions = (
  ...
  const allNamespaces = includeAllNamespaces
    ? [
        {
          children: ALL_NAMESPACES,
          groupVersionKind: modelToGroupVersionKind(NamespaceModel),
  ...
  // Return favorites first (if any), then regular namespaces
  return [...allNamespaces, ...favoriteOptions, ...regularOptions];
};
