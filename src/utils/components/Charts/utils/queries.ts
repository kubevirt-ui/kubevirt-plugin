...
export const getUtilizationQueries: GetUtilizationQueries = ({ duration, hubClusterName, obj }) => {
  ...
  const sumByCluster: string = isManagedCluster ? ', cluster' : '';
  ...
  return {
    ...
    [VMQueries.MIGRATION_DATA_PROCESSED]: `sum(max_over_time(kubevirt_vmi_migration_data_processed_bytes{name='${name}',namespace='${namespace}'${clusterFilter}}[${duration}])) BY (name, namespace${sumByCluster})`,
    [VMQueries.MIGRATION_DATA_REMAINING]: `sum(min_over_time(kubevirt_vmi_migration_data_remaining_bytes{name='${name}',namespace='${namespace}'${clusterFilter}}[${duration}])) BY (name, namespace${sumByCluster})`,
    [VMQueries.MIGRATION_DISK_TRANSFER_RATE]: `sum(avg_over_time(kubevirt_vmi_migration_memory_transfer_rate_bytes{name='${name}',namespace='${namespace}'${clusterFilter}}[${duration}])) BY (name, namespace${sumByCluster})`,
    [VMQueries.MIGRATION_MEMORY_DIRTY_RATE]: `sum(avg_over_time(kubevirt_vmi_migration_dirty_memory_rate_bytes{name='${name}',namespace='${namespace}'${clusterFilter}}[${duration}])) BY (name, namespace${sumByCluster})`,
