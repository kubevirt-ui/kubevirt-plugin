...
type SelectSecretProps = {
  ...
  onSelectSecret: (secret: IoK8sApiCoreV1Secret) => void;
  namespace: string;
};
...
const SelectSecret: React.FC<SelectSecretProps> = ({
  ...
  const onSelect = (event, newValue) => {
    onSelectSecret(secrets.find((secret) => secret?.metadata?.name === newValue));
  ...
  return (
    <Select
      ...
      hasInlineFilter
      selections={selectedSecret?.metadata?.name}
