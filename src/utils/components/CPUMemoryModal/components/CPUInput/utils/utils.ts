...
const parseValidationAnnotations = (
  ...
  const validations = parseJSONAnnotation<any[]>(annotations, 'vm.kubevirt.io/validations', {});

  if (!validations) {
    return { cores: 1, sockets: 1, threads: 1 };
  }

  const coresValidation = validations.find((value: any) => value.path?.includes('cpu.cores'));
  const socketsValidation = validations.find((value: any) => value.path?.includes('cpu.sockets'));
  const threadsValidation = validations.find((value: any) => value.path?.includes('cpu.threads'));
...
export const getCPULimitsFromTemplate = (template: any): Record<string, number> => {
