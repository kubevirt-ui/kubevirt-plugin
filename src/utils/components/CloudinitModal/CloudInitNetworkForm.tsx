...
export const CloudinitNetworkForm: React.FC<CloudinitNetworkFormProps> = ({
  ...
  useEffect(() => {
    ...
    const gateway4Invalid = gateway4 && !ipaddr.IPv4.isIPv4(gateway4);

    const gateway6Invalid = gateway6 && !ipaddr.IPv6.isIPv6(gateway6);

    setSubmitDisabled(gateway4Invalid || gateway6Invalid);
  }, [enableNetworkData, networkData?.gateway4, networkData?.gateway6, setSubmitDisabled]);
