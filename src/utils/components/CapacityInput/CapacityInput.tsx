...
const CapacityInput: FC<CapacityInputProps> = ({
  ...
  const validated: ValidatedOptions =
    !value || value <= 0 || value < minValue ? ValidatedOptions.error : ValidatedOptions.default;

  const getErrorValue = () => {
    if (value > 0) {
      return `less than ${minValue} ${unit}`;
    }
    ...
    >
      ...
      <FormGroupHelperText validated={validated}>
        {validated === ValidatedOptions.error &&
          t('Size cannot be {{errorValue}}', { errorValue: getErrorValue() })}
      </FormGroupHelperText>
    </FormGroup>
  );
};
