...
import { ensurePath, produceVMDisks } from '@catalog/utils/WizardVMContext';
...
type CloudinitNetworkFormProps = {
  ...
  updateVM: (updatedVM: V1VirtualMachine) => void;
  vm: V1VirtualMachine;
};
...
export const CloudinitNetworkForm: React.FC<CloudinitNetworkFormProps> = ({
  ...
  const onUpdateVM = React.useCallback(() => {
    ...
    updateVM(updatedVM);
  }, [vm, updateVM, cloudInitVolume?.name, cloudInitData, networkData, isBase64]);
  ...
  React.useEffect(() => {
    if (networkData) {
      ...
      if (hasChanged) onUpdateVM();
    } else {
      const updatedVM = produceVMDisks(vm, (draft) => {
  ...
  }, [setNetworkData, showForm, templateHasNetworkData, updateVM, vm]);
