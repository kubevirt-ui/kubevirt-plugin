...
const HighAvailabilityFeatureItem: FC<HighAvailabilityFeatureItemProps> = ({
  ...
  const { operatorDetailsMap, operatorsToInstall, updateInstallRequest } =
  ...
  const handleAlternativeOptionUpdate = (newCheckedState: boolean) => {
    ...
    if (newCheckedState) updateInstallRequest(operatorName, false);
  };
        ...
        <Split>
          <SplitItem className="high-availability-feature-item__checkbox-container">
            <Checkbox
              ...
              isDisabled={alternativeChecked}
              label={checkboxLabel}
              onChange={(_, checked) => updateInstallRequest(operatorName, checked)}
            />
          </SplitItem>
          <SplitItem>
