...
const Limits = ({ hyperConverge }) => {
  ...
  const updateLiveMigrationConfigWithDebounce: typeof updateLiveMigrationConfig =
    useDebounceCallback(updateLiveMigrationConfig, 500);
  const updateValueCluster = (value: number) =>
    updateLiveMigrationConfigWithDebounce(hyperConverge, value, MIGRATION_PER_CLUSTER);
  const updateValueNode = (value: number) =>
    updateLiveMigrationConfigWithDebounce(hyperConverge, value, MIGRATION_PER_NODE);
