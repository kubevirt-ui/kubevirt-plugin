...
const topConsumerQueries = {
  [Scope.NODE.getValue()]: {
    ...
    [Metric.STORAGE_READ_LATENCY.getValue()]: (numItemsToShow, duration) =>
      `sort_desc(topk(${numItemsToShow}, avg by (node)(rate(kubevirt_vmi_storage_read_times_seconds_total[${duration}]) / rate(kubevirt_vmi_storage_iops_read_total[${duration}]) > 0)))`,
    ...
    [Metric.STORAGE_WRITE_LATENCY.getValue()]: (numItemsToShow, duration) =>
      `sort_desc(topk(${numItemsToShow}, avg by (node)(rate(kubevirt_vmi_storage_write_times_seconds_total[${duration}]) / rate(kubevirt_vmi_storage_iops_write_total[${duration}]) > 0)))`,
  ...
  [Scope.PROJECT.getValue()]: {
    ...
    [Metric.STORAGE_READ_LATENCY.getValue()]: (numItemsToShow, duration) =>
      `sort_desc(topk(${numItemsToShow}, avg by (namespace)(rate(kubevirt_vmi_storage_read_times_seconds_total[${duration}]) / rate(kubevirt_vmi_storage_iops_read_total[${duration}]) > 0)))`,
    [Metric.STORAGE_THROUGHPUT.getValue()]: (numItemsToShow, duration) =>
    ...
    [Metric.STORAGE_WRITE_LATENCY.getValue()]: (numItemsToShow, duration) =>
      `sort_desc(topk(${numItemsToShow}, avg by (namespace)(rate(kubevirt_vmi_storage_write_times_seconds_total[${duration}]) / rate(kubevirt_vmi_storage_iops_write_total[${duration}]) > 0)))`,
    [Metric.VCPU_WAIT.getValue()]: (numItemsToShow, duration) =>
