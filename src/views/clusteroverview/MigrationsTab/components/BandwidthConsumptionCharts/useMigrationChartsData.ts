...
export const useMigrationChartsData: UseMigrationChartsData = (duration, currentTime, timespan) => {
  ...
  const [migrationsBandwidthConsumed, loadingBandwidth, errorBandwidth] = useFleetPrometheusPoll({
  ...
  const [proccessedMigrationsCount, loadingCount, errorCount] = useFleetPrometheusPoll({
    endpoint: PrometheusEndpoint?.QUERY_RANGE,
    endTime: currentTime,
  ...
  const migrationsCount = useMemo(
    () =>
      mapPrometheusValues(getPrometheusData(proccessedMigrationsCount), t('Running migrations')),
    [proccessedMigrationsCount, t],
  );
