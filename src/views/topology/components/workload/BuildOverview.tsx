...
import { fromNow } from '@console/internal/components/hooks/datetime';
...
import { BuildConfigOverviewItem, LogSnippet, Status } from '@console/shared';
        ...
        <div className="build-overview__item-title">
          ...
          {builds.length > MAX_VISIBLE && (
            <div>
              ...
              >
                {t('kubevirt-plugin~View all {{buildsLength}}', {
      ...
      {!(builds?.length > 0) ? (
        <li className="list-group-item">
          <span className="text-muted">
            {t('kubevirt-plugin~No Builds found for this Build Config.')}
          </span>
        </li>
      ) : (
        builds
...
export const BuildOverview: React.FC<BuildConfigsOverviewProps> = ({ buildConfigs }) => {
  ...
  return (
    <div className="build-overview">
      <SidebarSectionHeading text={t('kubevirt-plugin~Builds')} />
