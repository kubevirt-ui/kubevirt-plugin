...
export const useHealthChecksAlert = (element: GraphElement): DetailsResourceAlertContent | null => {
  ...
  const alertMessage =
    containersName?.length > 1
      ? t(
          'kubevirt-plugin~Not all Containers have health checks to ensure your application is running correctly.',
        )
      : t(
          'kubevirt-plugin~Container {{containersName}} does not have health checks to ensure your application is running correctly.',
          { containersName },
        );

  return showAlert
    ? {
        title: t('kubevirt-plugin~Health checks'),
        dismissible: true,
        content: (
          <>
            {alertMessage}{' '}
            <Link to={addHealthChecksLink}>{t('kubevirt-plugin~Add health checks')}</Link>
          </>
        ),
        variant: 'default',
      }
...
export const useResourceQuotaAlert = (element: GraphElement): DetailsResourceAlertContent => {
  ...
  return replicaFailure
    ? {
        title: t('kubevirt-plugin~Resource Quotas'),
        dismissible: true,
