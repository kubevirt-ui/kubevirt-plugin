...
const TopologyListViewNode: React.FC<TopologyListViewNodeProps & DispatchProps> = ({
  ...
  if (alerts?.length > 0) {
    ...
    alertIndicator = shouldHideMonitoringAlertDecorator(severityAlertType) ? null : (
      <Tooltip
        key="monitoringAlert"
        content={t('kubevirt-plugin~Monitoring alert')}
    ...
    >
      ...
      {children ? (
        <DataListContent
          ...
          aria-label={t('kubevirt-plugin~{{kindLabel}} {{label}}', {
            label: item.getLabel(),
        ...
        >
          <DataList
            aria-label={t('kubevirt-plugin~{{label}} sub-resources', { label: item.getLabel() })}
