...
import { modelFor, referenceFor, RouteKind } from '@console/internal/module/k8s';
  ...
  for (const nextResource of resources) {
    ...
    if (!resourceKind) {
      return Promise.reject(
        new Error(
          i18next.t(
            'kubevirt-plugin~Unable to update application, invalid resource type: {{kind}}',
            {
...
export const createTopologyResourceConnection = (
  ...
  if (!source || !target || source === target) {
    return Promise.reject(
      new Error(i18next.t('kubevirt-plugin~Can not create a connection from a node to itself.')),
    );
  }
