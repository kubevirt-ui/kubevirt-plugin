...
export const createSelfValidationCheckup = async (
  ...
  const jobData = selfValidationJob(name, namespace, checkupImage, selectedTestSuites, isDryRun);
  const jobName = jobData.metadata.name;

  // Create PVC first
  await k8sCreate({
    data: selfValidationPVC(jobName, namespace),
    model: PersistentVolumeClaimModel,
  });

  await k8sCreate({
    data: selfValidationConfigMap(namespace, name, checkupImage, selectedTestSuites, isDryRun),
...
export const rerunSelfValidationCheckup = async (
  ...
  const jobData = selfValidationJob(name, namespace, imageToUse, testSuites, isDryRun);
  const jobName = jobData.metadata.name;
