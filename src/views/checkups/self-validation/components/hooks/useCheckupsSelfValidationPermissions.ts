...
const useCheckupsSelfValidationPermissions = () => {
  ...
  useEffect(() => {
    ...
    checkAccess({
      ...
      .catch(() => {
        // If checkAccess fails, default to false to be safe
        // The installPermissions function will still try and provide a clear error
        setCanCreateClusterRoleBinding(false);
      })
      .finally(() => {
