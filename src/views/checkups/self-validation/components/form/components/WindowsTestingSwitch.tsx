...
const WindowsTestingSwitch: FC<WindowsTestingSwitchProps> = ({
  ...
  const windowsSwitch = (
    <Switch
      label={
        <>
          <span className="pf-v6-c-form__label-text">{t('Windows 11 testing')}</span>
          <HelpTextIcon
            bodyContent={t(
              'Enable Windows 11 golden image creation and testing. Requires accepting the Windows End User License Agreement (EULA).',
            )}
  ...
  return (
    <StackItem>
      {isTier2Selected ? (
        ...
        <Tooltip
          content={t('Windows 11 testing requires the Tier2 test suite to be enabled.')}
          position={PopoverPosition.right}
        >
          {windowsSwitch}
        </Tooltip>
