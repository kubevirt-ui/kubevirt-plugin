  ...
  (jobs || [])
    ?.filter((job) => {
      ...
      if (exactMatch) {
        ...
        // Extract base name by stripping -<number>-results suffix
        // CONFIGMAP_NAME format: <baseName>-<number>-results
        const baseName = name?.replace(/-\d+-results$/, '');
        return baseName === configMapName && job;
      }
    })
    .sort((a, b) =>
...
/**
 * Extracts the base ConfigMap name and namespace from a Job's environment variables.
 *
 ...
 * This function strips the `-<number>-results` suffix to extract the original base
 * ConfigMap name used for the checkup.
 *
 * @param job - The Kubernetes Job to extract ConfigMap info from
 * @returns An object with the base name and namespace, or null if not found
 */
