...
import { type BootableResource, type BootableVolumeMetadata } from './types';
...
export const getPreferenceReadableOS = (
  ...
  const preferenceObject = preferences?.find(
    (preference) =>
      preference?.metadata?.name === preferenceLabelValue && getCluster(preference) === objCluster,
  );
...
export const deleteBootableVolumeMetadata = (obj: BootableResource): (() => Promise<void>) => {
...
export const changeBootableVolumeMetadata =
  (obj: BootableResource, metadata: BootableVolumeMetadata): (() => Promise<void>) =>
  async (): Promise<void> => {
