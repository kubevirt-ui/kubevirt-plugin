...
import { type EditMigrationPolicyInitialState } from './utils/constants';
import {
...
const MigrationPolicyEditModal: FC<MigrationPolicyEditModalProps> = ({ isOpen, mp, onClose }) => {
  ...
  const setStateField =
    (field: keyof EditMigrationPolicyInitialState) =>
    (
      value:
        | ((
            prev: EditMigrationPolicyInitialState[keyof EditMigrationPolicyInitialState],
          ) => EditMigrationPolicyInitialState[keyof EditMigrationPolicyInitialState])
        | EditMigrationPolicyInitialState[keyof EditMigrationPolicyInitialState],
    ): void => {
  ...
  const onSubmit = async (
    ...
    if (updatedMP?.metadata?.name !== mp?.metadata?.name) {
      await kubevirtK8sCreate({ cluster, data: updatedMP, model: MigrationPolicyModel });
      await kubevirtK8sDelete({ cluster, model: MigrationPolicyModel, resource: mp });

      if (lastPolicyPathElement === mp?.metadata?.name) {
        navigate(getMigrationPolicyURL(getName(updatedMP), cluster));
      } else {
        navigate(migrationPoliciesBaseURL);
      }
      return;
    }
