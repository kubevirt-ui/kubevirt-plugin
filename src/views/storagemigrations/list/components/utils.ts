...
export const getMigrationPercentage = (
  ...
  const runningMigrations =
    storageMigrationPlan?.status?.namespaces?.reduce(
      (acc, namespace) => acc + namespace?.inProgressMigrations?.length,
      0,
    ) || 0;

  const totalMigrations = successfulLiveMigration + runningMigrations;
  if (totalMigrations === 0) return 0;

  return ((successfulLiveMigration + runningMigrations) / totalMigrations) * 100;
};
