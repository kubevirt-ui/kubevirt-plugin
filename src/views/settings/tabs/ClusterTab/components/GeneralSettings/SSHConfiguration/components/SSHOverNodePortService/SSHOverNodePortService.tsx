...
import { Content, ContentVariants, TextInput } from '@patternfly/react-core';
...
const SSHOverNodePortService: FC<SSHOverNodePortServiceProps> = ({ newBadge }) => {
  ...
  return (
    <ExpandSectionWithCustomToggle
      customContent={newBadge ? <NewBadge /> : <div />}
      helpTextContent={t(
    ...
    >
      ...
      <Content className="project-tab__main--help pf-v6-u-mt-sm" component={ContentVariants.small}>
        {t('To enable SSH over NodePort service, Node address needs to be filled in.')}
      </Content>
    </ExpandSectionWithCustomToggle>
  );
};
