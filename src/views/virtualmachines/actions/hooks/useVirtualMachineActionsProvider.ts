...
const useVirtualMachineActionsProvider: UseVirtualMachineActionsProvider = (
  ...
  const actions: ActionDropdownItemType[] = React.useMemo(() => {
    ...
    const migrateStorage = VirtualMachineActionFactory.migrateStorage(vm, navigate);
    ...
    return [
      ...
      VirtualMachineActionFactory.snapshot(vm, createModal),
      {
        id: 'migration-menu',
        label: 'Migration',
        options: [migrateOrCancelMigrationCompute, migrateStorage],
      } as ActionDropdownItemType,
  ...
  }, [vm, vmim, isSingleNodeCluster, navigate, createModal, virtctlCommand]);
