...
import { migrateVM } from './utils/utils';
...
const ComputeMigrationModal: FC<ComputeMigrationModalProps> = ({ isOpen, onClose, vm }) => {
  ...
  const initiateMigration = () => migrateVM(vm);

  return (
    <Modal height="500px" isOpen={isOpen} onClose={onClose} variant={ModalVariant.large}>
      <ModalHeader>
        <Stack hasGutter>
          <StackItem>
      ...
      <ModalBody>
        <Stack hasGutter>
          ...
          {migrationOption === MigrationOptions.MANUAL && (
            <StackItem>
              <NodesTable handleNodeSelection={handleNodeSelection} selectedNode={selectedNode} />
            </StackItem>
          )}
        </Stack>
      </ModalBody>
