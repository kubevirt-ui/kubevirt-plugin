...
export const createBulkVirtualMachineActionFactory = (
  ...
  migrateCompute: (
    ...
    const allLiveMigratable = !isEmpty(vms) && vms.every(isLiveMigratable);
    const nonMigratableVMs = vms?.filter((vm) => !isLiveMigratable(vm)) || [];
    const nonMigratableVMNames = nonMigratableVMs.map((vm) => getName(vm)).join(', ');
    ...
    return {
      ...
      disabled: !allLiveMigratable,
      disabledTooltip: allLiveMigratable
        ? getNoPermissionTooltipContent(t)
        : t('The following selected VMs are not eligible for live migration: {{vmNames}}', {
            vmNames: nonMigratableVMNames,
          }),
