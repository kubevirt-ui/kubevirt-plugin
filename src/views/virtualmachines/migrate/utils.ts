...
const getBlankDataVolume = (
  ...
  metadata: {
    name: `clone-${getRandomChars()}-${originName}`.substring(0, MAX_NAME_LENGTH),
    namespace,
  },
  spec: {
    source: {
...
const createBlankDataVolumes = async (
  ...
  const rejectedRequest = dataVolumesCreationResults.find((result) => result.status === 'rejected');

  const fulfilledRequests = dataVolumesCreationResults.filter(
    (result) => result.status === 'fulfilled',
  ) as PromiseFulfilledResult<V1beta1DataVolume>[];
  ...
  if (rejectedRequest) {
    ...
    throw new Error((rejectedRequest as PromiseRejectedResult).reason);
  }
