...
import { type RowReducerFilter } from '@openshift-console/dynamic-plugin-sdk';
import { VirtualMachineRowFilterType } from '@virtualmachines/utils';

export const getOSFilter = (t: TFunction): RowReducerFilter<V1VirtualMachine> => {
  const getOSName = (obj: V1VirtualMachine): string | undefined => {
    ...
    const termStartsWithOSName = (osPrefix: string, term?: string): boolean | undefined =>
      term?.toLowerCase()?.startsWith(osPrefix.toLowerCase());
