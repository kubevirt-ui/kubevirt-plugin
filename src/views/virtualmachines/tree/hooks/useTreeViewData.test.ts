...
jest.mock('@kubevirt-utils/hooks/useKubevirtWatchResource', () => ({
  __esModule: true,
...
const getWatchedNamespaces = (): string[] => [
  ...new Set(
    (useK8sWatchResources as jest.Mock).mock.calls.flatMap(
      ([resourcesToWatch]: [Record<string, unknown>]) => Object.keys(resourcesToWatch ?? {}),
    ),
  ),
];
