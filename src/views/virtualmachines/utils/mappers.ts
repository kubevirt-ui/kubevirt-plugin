...
export const getVMIFromMapper = (VMIMapper: VMIMapper, vm: V1VirtualMachine) =>
  VMIMapper?.mapper?.[getCluster(vm) || NO_MULTICLUSTER_KEY]?.[getNamespace(vm)]?.[getName(vm)];
...
export const getVMIMFromMapper = (
  ...
  namespace: string,
  cluster?: string,
) => VMIMMapper?.mapper?.[cluster || NO_MULTICLUSTER_KEY]?.[namespace]?.[name];
...
export const convertIntoPVCMapper = (pvcs: IoK8sApiCoreV1PersistentVolumeClaim[]): PVCMapper => {
  return (pvcs || []).reduce((acc, pvc) => {
    const cluster = getCluster(pvc) || NO_MULTICLUSTER_KEY;
