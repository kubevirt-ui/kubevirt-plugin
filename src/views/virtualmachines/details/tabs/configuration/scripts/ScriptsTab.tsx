...
const ScriptsTab: FC<VirtualMachineScriptPageProps> = ({ obj: vm }) => {
  ...
  const accessCredentials = useMemo(() => getAccessCredentials(vm), [vm]);
  const [canUpdateVM] = useAccessReview(
    asAccessReview(VirtualMachineModel, vm, 'update' as K8sVerb) || {},
  );
            ...
            <VirtualMachineDescriptionItem
              descriptionData={
                <Stack hasGutter>
                  ...
                  <SecretNameLabel
                    secretName={accessCredentials?.[0]?.sshPublicKey?.source?.secret?.secretName}
                  />
                </Stack>
              }
              onEditClick={() =>
