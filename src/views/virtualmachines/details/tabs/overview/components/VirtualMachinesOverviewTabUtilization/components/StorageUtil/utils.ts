import { type V1VirtualMachineInstanceFileSystem } from '@kubevirt-ui-ext/kubevirt-api/kubevirt';
...
export const filterWritableFilesystems = (
  ...
  filesystems?.filter(
    (filesystem) => !EXCLUDED_FILESYSTEM_TYPES.includes(filesystem?.fileSystemType),
  );
