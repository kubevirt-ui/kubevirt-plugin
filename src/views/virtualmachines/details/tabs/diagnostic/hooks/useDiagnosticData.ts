...
const volumeSnapshotStatusesTransformer = (
  ...
  volumeSnapshotStatuses.map((vss) => {
    const parsedReason = parseDiagnosticReason(vss?.reason, vss?.name);
    ...
    return {
      ...
      metadata: {
        condition: 'Other',
        name: parsedReason.name,
        type: DiagnosticCategory.Storage,
      },
      reason: parsedReason.reason,
      severity:
        vss?.name === CLOUDINITDISK || isContainerDiskVolume(vss?.name, volumes)
          ? 'healthy'
          : getSnapshotSeverity(vss?.enabled),
      status: vss?.enabled,
    };
  });
