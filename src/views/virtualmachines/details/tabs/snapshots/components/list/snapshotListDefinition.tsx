...
const CreatedCell: FC<CreatedCellProps> = ({ row }) => (
...
type LastRestoredCellProps = {
  callbacks: SnapshotListCallbacks;
  row: V1beta1VirtualMachineSnapshot;
};

const LastRestoredCell: FC<LastRestoredCellProps> = ({ callbacks, row }) => {
  const relevantRestore = callbacks.restores?.[getName(row)];
  return <Timestamp timestamp={relevantRestore?.status?.restoreTime} />;
};

type IndicationsCellProps = {
  row: V1beta1VirtualMachineSnapshot;
};

const IndicationsCell: FC<IndicationsCellProps> = ({ row }) => (
  <IndicationLabelList snapshot={row} />
);
...
export const getSnapshotListColumns = (
  ...
  },
  {
    key: 'indications',
    label: t('Indications'),
    renderCell: (row) => <IndicationsCell row={row} />,
  },
  {
    key: 'actions',
    label: '',
