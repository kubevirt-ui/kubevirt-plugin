...
const main = async (): Promise<void> => {
  ...
  // API Server
  results.push(await check('API server reachability', async () => {
    await client.coreV1.listNamespacedPod({ namespace: 'default', limit: 1 });
    return 'API server responding';
  }));

  // Node readiness
  results.push(await check('node readiness', async () => {
  ...
  // HCO Available
  results.push(await check('HCO Available condition', async () => {
    ...
    if (available?.status !== 'True') {
      throw new Error(`HCO Available=${available?.status ?? 'unknown'}: ${available?.message ?? ''}`);
    }
  ...
  for (const component of ['virt-api', 'virt-controller', 'virt-handler']) {
    results.push(await check(`${component} pods`, async () => {
  ...
  results.push(await check(`ARC AutoscalingRunnerSet in ${ARC_RUNNERS_NS}`, async () => {
    try {
  ...
  // ARC listener pod
  results.push(await check(`ARC listener pod in ${ARC_CONTROLLER_NS}`, async () => {
  ...
  results.push(await check('default StorageClass', async () => {
    const { items } = await client.kc.makeApiClient(
      (await import('@kubernetes/client-node')).StorageV1Api,
    ).listStorageClass();

    const defaultSc = items.find((sc) =>
  ...
  // Console route
  results.push(await check('console route accessible', async () => {
