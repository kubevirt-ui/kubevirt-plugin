...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    if ((err as { statusCode?: number }).statusCode !== 404) {
      console.warn(
        `Could not delete secret ${testSecretName}: ${err instanceof Error ? err.message : err}`,
      );
    }
  }
  ...
  for (const { group, version, plural, name } of namedDeletes) {
    try {
      ...
      if ((err as { statusCode?: number }).statusCode !== 404) {
        console.warn(
          `Could not delete ${plural}/${name}: ${err instanceof Error ? err.message : err}`,
        );
      }
    }
  }
  ...
  for (const { group, version, plural, name } of clusterDeletes) {
    try {
      ...
      if ((err as { statusCode?: number }).statusCode !== 404) {
        console.warn(
          `Could not delete cluster ${plural}/${name}: ${err instanceof Error ? err.message : err}`,
        );
      }
    }
  }
