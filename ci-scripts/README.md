...
PR entry points (thin gates that only dispatch the workflow above):
  ...
  ├── retest-e2e.yml                → /retest-e2e (always dispatches fresh)
  ├── retest-failed.yml             → /retest-failed (dispatches only if currently failing/stale; also
  │                                    re-runs failed ci_checks.yml/pr_validation.yml jobs -- see below)
...
pr_validation_commands.yml    → /lgtm, /approve, /hold (+ cancel), /ai-approved, /ci-approved, /i18n-approved, /recheck-jira
...
- **`/retest-failed` spans all three CI surfaces (`ci_checks.yml`, `pr_validation.yml`, Hot Cluster E2E) and only acts on whichever is currently failing/stale -- unlike `/retest-e2e`, it never forces a fresh run of something already passing.** For `ci_checks.yml`/`pr_validation.yml`: reruns the latest run at the head SHA via `reRunWorkflowFailedJobs`, but only if its `conclusion` is `failure`/`timed_out` (still-running, already-passing, and `cancelled` runs are all left alone -- `reRunWorkflowFailedJobs` targets jobs whose own conclusion is `failure`, and a cancelled run typically has none, so calling it there 403s). For E2E: reads the current `Run Gating Tests` check-run for that head SHA via the **ambient** `GITHUB_TOKEN` (not the bot app, which has no Checks permission to grant, and reads aren't subject to the fork-PR write restriction anyway) and dispatches fresh (same inputs as `/retest-e2e`) only if that check is missing or not `success`. Like every other auto-dispatcher, it respects an active `/hold-e2e` hold -- only `/retest-e2e` lifts one.
...
Commands in `pr_validation_commands.yml` (`.github/scripts/src/validation/commands/{lgtm,approve,hold}.ts`), alongside `/ai-approved`/`/ci-approved`/`/i18n-approved`/`/recheck-jira`. `parse-command.ts` accepts an optional trailing `cancel` (e.g. `/lgtm cancel`); word-boundary matching rejects `/hold-e2e` as `/hold`.
...
| `/lgtm cancel`    | `lgtm` (removed); also always removes `approved` | Any write-access collaborator, including the PR's own author                                  |
...
`pr_validation.yml` already runs Jira/AI-config/CI-scripts validation on every `synchronize`; it now also removes both `lgtm` and `approved` on that same event (`clear-stale-approval.ts`, wired into `pr-validation/index.ts`), mirroring Prow's `lgtm` plugin invalidating a stale review on a new commit. Without this, a PR that once reached `lgtm`+`approved` stayed merge-pool eligible through any number of later unreviewed pushes -- combined with native auto-merge, that meant a completely different diff could auto-merge under a stale approval. Only on `synchronize`: `opened`/`reopened`/`edited` can't carry a stale review from a different diff. Best-effort and idempotent (`removeLabel` no-ops on a missing label), so it's safe to call on every push regardless of whether either label is present. `auto-merge.yml`'s own `unlabeled` trigger re-evaluates `Merge Gate` immediately once the labels are actually removed, closing the brief window between the push landing and this job removing them.
...
Purely informational, PR-list-visible mirror of Hot Cluster E2E's latest _real_ result -- **not** a merge gate (`isMergePoolPr` doesn't read these; the required "Run Gating Tests" check-run is still the actual gate). Added by a new step in `hot-cluster-e2e.yml`'s `verify-gating-tests` job, right after "Publish result for PR": `success` -> `e2e-passed` (+ remove `e2e-failed`); `failure` -> `e2e-failed` (+ remove `e2e-passed`); `neutral` (held/non-real results) -> skipped entirely there, leaving whichever label already reflects the last real result untouched. Label writes go through the `kubevirt-plugin-bot` App token (`permission-issues: write` + `permission-pull-requests: write`), same reason as `/hold-e2e`'s `e2e-hold` -- not `GITHUB_TOKEN`.

The one exception to "neutral leaves the label untouched" is `retest-stale-gating.yml`'s own `mark-stale` job: every push to `main` clears both `e2e-passed`/`e2e-failed`, for every open PR the job processes -- unconditionally, not tied to whether it also happened to (re-)mark that PR's `Run Gating Tests` check stale (`should_mark`). `should_mark` only governs the check-run itself (e.g. left alone while held, or while a real run is already in flight) -- it says nothing about whether the label is still valid, and `main` advancing invalidates it regardless. `if: always()`, so an earlier failure in the same job (e.g. the stale-check-run lookup erroring) doesn't also skip this unrelated cleanup, which only needs `matrix.pr_number`. Applies to every open PR targeting `main`, not just merge-pool PRs; a merge-pool PR gets a fresh real result soon after from the dispatched retest, while a non-pool PR simply has no e2e label until a push, `/retest-e2e`, or pool-entry supplies one.

Both labels are also cleared on `synchronize`, regardless of `e2e-hold` state -- `hot-cluster-e2e-pr-gate.yml`'s `dispatch` job (hold absent) and its `republish-hold` job (hold present) each remove whichever of `e2e-passed`/`e2e-failed` is present for the new HEAD, via the shared [`clear-e2e-result-labels`](../.github/actions/clear-e2e-result-labels/action.yml) composite action -- so the PR list can't keep showing a previous commit's result as if it applied to untested code, whether or not the PR happens to be held. Only on `synchronize`: `opened`/`reopened` have no prior-diff result to invalidate. Best-effort (`continue-on-error: true`) and idempotent (404 on a missing label is swallowed), same pattern as `clear-stale-approval` for `lgtm`/`approved`. Composite-action steps run inline in the calling job, so this adds no new job/check to the merge box.
...
- **Dismissing a review does not clear `lgtm`/`approved`.** `pr_review_commands.yml` only listens for `pull_request_review: [submitted]`; GitHub's "Dismiss review" action fires a separate `dismissed` action that isn't handled. An Approve review that granted `lgtm`+`approved` and is later dismissed by a maintainer leaves both labels in place, now backed by a review GitHub itself no longer shows as active. Lower severity than the push/cancel staleness cases above -- it requires an explicit maintainer dismissal, not just an ordinary write-collaborator command -- so it's left as a known gap rather than fixed here.

## CI-Scripts, AI-Config & Translations Validation (OWNERS-Gated Merge Blocking)

AI-config, CI-scripts, and i18n path gates (`.github/scripts/src/validation/ai-config-validation/`, `.github/scripts/src/validation/ci-scripts-validation/`, `.github/scripts/src/validation/i18n-validation/`, all thin wrappers around the shared `.github/scripts/src/validation/pr-path-validation/` core) block merging until an OWNERS-listed reviewer signs off on changes to sensitive paths. Blocking is via `do-not-merge/*` labels consumed by **Merge Gate** -- these gates do **not** publish their own commit-status contexts.

| Gate       | Protected paths                                                                                                                                                                                                         | Alert label          | Block label                      | Reviewed label        | Skip label              | Approve command  |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | -------------------------------- | --------------------- | ----------------------- | ---------------- |
| AI-config  | `.cursor/`, `.claude/`, `.vscode/`, `.gemini/`, `.codex/`, `.windsurf/`, `.coderabbit.yaml`, `AGENTS.md`, `.cursorrules`, `.github/copilot-instructions.md`, `.github/scripts/`, the two `pr_validation*.yml` workflows | `ai-config-changed`  | `do-not-merge/ai-config-review`  | `ai-config-reviewed`  | `skip-ai-config-check`  | `/ai-approved`   |
| CI-scripts | `.github/`, `ci-scripts/`, `Dockerfile`, `playwright-runner-hc-e2e.sh` (confirmed CI-invoked; other root-level scripts checked and ruled out as local-dev-only)                                                         | `ci-scripts-changed` | `do-not-merge/ci-scripts-review` | `ci-scripts-reviewed` | `skip-ci-scripts-check` | `/ci-approved`   |
| i18n       | `locales/`                                                                                                                                                                                                              | `i18n-changed`       | `do-not-merge/i18n-review`       | `i18n-reviewed`       | `skip-i18n-check`       | `/i18n-approved` |

All three run unconditionally on every PR (`pull_request_target: [opened, synchronize, reopened, edited]` in `pr_validation.yml`, no path filter) -- the path-sensitivity decision happens inside the script, not as a workflow-level filter, so the common "doesn't touch these paths" case only clears any stale alert/block labels. `/ai-approved`/`/ci-approved`/`/i18n-approved` (via `pr-commands.yml`, `issue_comment`-triggered) all check the commenter against **`.github/OWNERS`** (`no_parent_owners: true`, kept in sync with `.cursor/OWNERS`) at the PR's _base_ ref, never the PR's own branch -- so a PR can't self-approve by editing OWNERS in its own diff. A new commit to a sensitive path clears the "reviewed" label and re-blocks, same as `ai-config-reviewed` always did.

`pr_validation.yml` runs Jira (commit status) plus AI-config / CI-scripts / i18n (labels only) from a single `pr-validation` job/script (`src/validation/pr-validation/index.ts`) -- one checkout, one PR-files fetch shared between the path validations, and each check still isolated from the others' failures.

- **Merge blocking for path gates is labels + Merge Gate, not per-gate commit statuses.** `do-not-merge/ai-config-review` / `do-not-merge/ci-scripts-review` / `do-not-merge/i18n-review` make Merge Gate fail; Merge Gate is a required status check. The path gates no longer publish `ai-config-validation` / `ci-scripts-validation` / `i18n-validation` contexts.
- **The reviewed/skip labels alone are not the trust boundary -- label-gate closes that gap.** `runPathValidation`'s `passed` computation only checks label _presence_, which can't distinguish a label added by `/ai-approved`/`/ci-approved`/`/i18n-approved` from one added directly via the GitHub UI by anyone with triage/write access. The label-gate route in `pr-validation.yml` checks the label-applier against `.github/OWNERS` and strips the label immediately if they're not listed, then re-runs validation so labels reflect the corrected state.
- **Bot-applied reviewed labels must be exempted from that same check, or the fix breaks the thing it protects.** `/ai-approved`/`/ci-approved`/`/i18n-approved` add the label via the `kubevirt-plugin-bot` token -- the resulting `labeled` event's `sender` is the bot account, not the commenter who was already checked against OWNERS in `commands/approve.ts`. `verify-review-labels` exempts only the exact `kubevirt-plugin-bot[bot]` login, not any `*[bot]` sender -- trusting every bot/app with label-write access would let a third-party app bypass review on the reviewed _and_ skip-\* labels alike.
- **The AI-config/CI-scripts overlap on `.github/` is intentional, not an oversight.** `AI_CONFIG.relatedAutomationPaths`/`relatedAutomationPrefixes` (`.github/workflows/pr_validation*.yml`, `.github/scripts/`) is a strict subset of `CI_SCRIPTS_CONFIG`'s `.github/` prefix, so a change to those specific files needs both `/ai-approved` and `/ci-approved`. Kept as defense-in-depth.
- **Fine-grained tokens report `issues: write` OR `pull-requests: write` as sufficient for labels/comments -- live behavior on a PR target says otherwise.** Same gotcha as `/retest-e2e`'s bot token above: labels/comments on a PR need `pull-requests: write` specifically, not `issues: write` alone, even though the docs list either as acceptable. `pr-commands.yml` routes `/ai-approved`/`/ci-approved`/`/i18n-approved`'s label/reaction calls through the `kubevirt-plugin-bot` app token (requesting both scopes) for the same fork-PR `GITHUB_TOKEN`-read-only reason documented above, but keeps `jira-validation` commit-status calls **and** the `.github/OWNERS` lookup itself (`repos.getContent`, needs `contents: read`) on the ambient `GITHUB_TOKEN` via a second, ambient-scoped Octokit client (`GitHubConfig.statusToken`, `createStatusOctokit` in `github-repo.ts`) -- the bot app has neither `statuses` nor `contents` permission and won't be granted either, and both scopes already work fine on a fork PR via the ambient token. Missing this split for the OWNERS lookup specifically would silently reject every legitimate `/ai-approved`/`/ci-approved` (the bot token's 403 on `getContent` is caught and treated as "untrusted", not surfaced as an error) -- caught in this session's own security review before it shipped.
- **`pr-commands.yml` uses `cancel-in-progress: false` on command concurrency groups.** Rapid successive comments (`/lgtm` then `/approve`, or `/ai-approved` then `/ci-approved`) queue rather than cancel mid-flight.
