...
The hot-cluster pipeline manages its own required check ("Run Gating Tests") and an informational "Hot Cluster E2E Progress" status across several workflows (`hot-cluster-e2e.yml`, `retest-e2e.yml`, `retest-stale-gating.yml`) and the `publish-gating-check` action. A few GitHub behaviors drive the design:
