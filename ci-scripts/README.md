...
The hot-cluster pipeline manages its own required check ("Run Gating Tests") and an informational "Hot Cluster E2E Progress" status across several workflows (`hot-cluster-e2e.yml`, `retest-e2e.yml`, `cancel-e2e.yml`, `retest-stale-gating.yml`, `retest-on-pool-entry.yml`, `ok-to-test-gate.yml`) and the `publish-gating-check` action. A few GitHub behaviors drive the design:
...
- **Bot labels don't trigger `hot-cluster-e2e.yml` at all -- only `ok-to-test` does, and only indirectly.** `labeled` used to be a direct trigger, so every bot label (`lgtm`, `approved`, `jira/valid-reference`, etc.) re-ran the full 5-job graph just to skip all 5 via `if:` -- 5 permanently-"Skipped" rows in the merge box per irrelevant label, since GitHub creates a native check-run per job the moment a workflow triggers, regardless of that job's own `if:`. There's no way to filter `on.pull_request_target.types` by which label was added, so the fix is `ok-to-test-gate.yml`: a separate one-job workflow that's now the only thing listening for `labeled`, dispatching `hot-cluster-e2e.yml` via `workflow_dispatch` (`pr_number`) only when the label is actually `ok-to-test`. This caps the noise at one skipped row (the gatekeeper's own job) per irrelevant label, instead of five, and `hot-cluster-e2e.yml`'s own `on:` no longer includes `labeled` at all.
