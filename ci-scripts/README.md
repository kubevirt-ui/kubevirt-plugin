...
- **Publish "Run Gating Tests" with `GITHUB_TOKEN` (github-actions app_id 15368), not the bot App.** Branch protection pins that required check to github-actions. Publishing with `kubevirt-plugin-bot` (app_id 4257649) produces a green check the merge box can show, but the required check stays forever "Expected -- waiting for status", leaving auto-merge `BLOCKED` (confirmed on Dependabot PR #4497). `closeOrphanedCheckRuns` also rewrites older completed failures/neutrals to `cancelled` when a newer result lands, and `auto-merge.yml` uses `cancel-in-progress: true` plus the same supersede path for "Evaluate Merge Eligibility", so stacked completed failures cannot keep the status rollup red after a later success.
