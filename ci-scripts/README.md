...
- **`/retest-failed` is the one command that spans all three CI surfaces (`ci_checks.yml`, `pr_validation.yml`, and Hot Cluster E2E), and only acts on whichever is currently failing/stale.** Unlike `/retest-e2e`, it never forces a fresh run of something already passing: for `ci_checks.yml`/`pr_validation.yml` it finds the latest run at the PR's current head SHA and calls `reRunWorkflowFailedJobs` only if that run's `conclusion` is `failure`/`timed_out` (a still-running or already-passing run is left alone; a `cancelled` run is also left alone -- `reRunWorkflowFailedJobs` only targets jobs whose own conclusion is `failure`, and a cancelled run typically has none, so calling it there would 403 into the outer error path instead of a clean per-check skip); for E2E it reads the current "Run Gating Tests" check-run for that head SHA (same `checks.listForRef` + latest-by-`started_at` pattern as `retest-stale-gating.yml`/`retest-on-pool-entry.yml`) and only `createWorkflowDispatch`es (`pr_number`/`base_ref`/`skip_pool_check: true`, same inputs as `/retest-e2e`) if that check is missing or completed with anything other than `success`. It reads that check-run via the **ambient** `GITHUB_TOKEN`, not the bot app token -- the bot app is only installed with Issues + Pull requests write (see below), so it has no Checks permission to grant a token at all, and reads are unaffected by the fork-PR write restriction regardless (same reasoning as the `checks`/`statuses` bullet below). It also unconditionally respects an active `/hold-e2e` hold exactly like every other auto-dispatcher (`hot-cluster-e2e-pr-gate.yml`, `ok-to-test-gate.yml`, `retest-stale-gating.yml`) -- `/retest-failed` never lifts a hold, only `/retest-e2e` does.
...
- On failure, names the **specific** unmet condition(s) rather than one generic sentence -- e.g. `Not eligible: missing lgtm, held via /hold-e2e` -- via `getMergePoolBlockers` in [`hot-cluster/js/is-merge-pool-pr.cjs`](hot-cluster/js/is-merge-pool-pr.cjs), which splits `isMergePoolPr`'s single boolean into which of `missingLgtm`/`missingApproved`/`blockingLabels` actually failed. This lands in the `::error title=...::` annotation on the `Merge Gate` check's own run page, so an active `/hold-e2e` (or any other blocker) is identifiable without cross-referencing the separately-published, confusingly-grouped `Run Gating Tests` check elsewhere in the merge box.
...
The one exception to "neutral leaves the label untouched" is `retest-stale-gating.yml`'s own `mark-stale` job: every push to `main` clears both `e2e-passed`/`e2e-failed`, for every open PR the job processes -- unconditionally, not tied to whether it also happened to (re-)mark that PR's `Run Gating Tests` check stale (`should_mark`). `should_mark` only governs the check-run itself (e.g. left alone while held, or while a real run is already in flight) -- it says nothing about whether the label is still valid, and `main` advancing invalidates it regardless. Applies to every open PR targeting `main`, not just merge-pool PRs; a merge-pool PR gets a fresh real result soon after from the dispatched retest, while a non-pool PR simply has no e2e label until a push, `/retest-e2e`, or pool-entry supplies one.

Both labels are also cleared on `synchronize` -- `hot-cluster-e2e-pr-gate.yml`'s `dispatch` job removes whichever of `e2e-passed`/`e2e-failed` is present, right before dispatching the fresh run for the new HEAD, so the PR list can't keep showing a previous commit's result as if it applied to code that hasn't been tested yet. Only on `synchronize`: `opened`/`reopened` have no prior-diff result to invalidate. Best-effort (`continue-on-error: true`) and idempotent (404 on a missing label is swallowed), same pattern as `clear-stale-approval` for `lgtm`/`approved`.
