...
## Tide Removal & Native Auto-Merge

`kubevirt-ui/kubevirt-plugin` no longer uses Tide to merge PRs. Merge-gating now lives entirely in this repo: `.github/workflows/auto-merge.yml` toggles GitHub's own native auto-merge from the existing `lgtm`/`approved`/hold labels (via `ci-scripts/hot-cluster/is-merge-pool-pr.cjs`), and a required **"Merge Gate"** check-run is the real backstop -- branch protection blocks merge on it directly, independent of whether the GraphQL enable/disable call itself succeeds. See the "Tide Required-Context Gotcha" section right below for why this replaced Tide: its config repeatedly drifted out of sync with this repo's actual required checks and labels (PR #4356 merged "despite BLOCKED"; PR #4349's `/hold-e2e` didn't block Tide either), and maintaining that parity in a separate repo (`openshift/release`) every time a new required check or hold label was introduced wasn't worth it.

### Migration steps (for reference -- already applied to this repo)

1. **Land the replacement first.** `auto-merge.yml`, the required `Merge Gate` check, and the explicit Checks-API publishing for `ai-config-validation`/`ci-scripts-validation` (CNV-93066) all need to be live on `main` -- and `main` branch protection needs to require every check that must gate merge (`Run Gating Tests`, `ai-config-validation`, `ci-scripts-validation`, `build`, `test`, `i18n`, `actionlint`, `Merge Gate`) -- _before_ removing Tide, so there's no gap where neither system enforces anything.
2. **Remove Tide's config for this repo in `openshift/release`.** Delete `core-services/prow/02_config/kubevirt-ui/kubevirt-plugin/_prowconfig.yaml` -- its entire content is the `tide:` block scoped to this one repo, so deleting it removes `kubevirt-ui/kubevirt-plugin` from Tide's queries without touching any other repo's Tide config, and without touching this repo's separate `_pluginconfig.yaml` (a different file -- see step 3).
3. **Do _not_ disable the OpenShift CI bot (`openshift-ci`/`openshift-ci-robot`) -- only Tide's merge role goes away.** That bot also drives `_pluginconfig.yaml`'s `lgtm`/`approve`/`hold`/`label`/`cherrypick`/`needs-rebase`/`jira-lifecycle-plugin` plugins, which `auto-merge.yml` itself still depends on (`isMergePoolPr` reads exactly the `lgtm`/`approved`/`hold`/`do-not-merge/*` labels those plugins apply). Removing this bot would break label automation entirely, not just Tide -- it is a completely separate Prow component (`hook`) from Tide, configured in a different file.
4. **`openshift-merge-bot` naturally stops merging this repo's PRs once step 2 lands**, since Tide will no longer evaluate `kubevirt-ui/kubevirt-plugin` at all. For a hard guarantee beyond "Tide won't try" (defense in depth, not required for correctness), also remove that GitHub App's installation access from this repo specifically: Settings → Integrations → GitHub Apps → `openshift-merge-bot` → Configure → remove `kubevirt-ui/kubevirt-plugin` from its repository access.
5. **Verify.** Re-create the PR #4356 (`mergeStateStatus: BLOCKED`) and PR #4349 (`e2e-hold` + a failing required check) scenarios against the new setup and confirm merge stays blocked with no bot bypass -- and confirm a real `lgtm`+`approved` PR with everything green now merges with no one clicking "Merge" or "Enable auto-merge" manually.
