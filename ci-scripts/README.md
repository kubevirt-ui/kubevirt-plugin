...
`ai-config-validation`, `ci-scripts-validation`, and `i18n-validation` (`.github/scripts/src/validation/ai-config-validation/`, `.github/scripts/src/validation/ci-scripts-validation/`, `.github/scripts/src/validation/i18n-validation/`, all thin wrappers around the shared `.github/scripts/src/validation/pr-path-validation/` core) block merging until an OWNERS-listed reviewer signs off on changes to sensitive paths:
...
| `i18n-validation`       | `locales/`                                                                                                                                                                                                              | `i18n-changed`       | `do-not-merge/i18n-review`       | `i18n-reviewed`       | `skip-i18n-check`       | `/i18n-approved` |

All three checks run unconditionally on every PR (`pull_request_target: [opened, synchronize, reopened, edited]` in `pr_validation.yml`, no path filter) -- the path-sensitivity decision happens inside the script, not as a workflow-level filter, so the common "doesn't touch these paths" case always resolves to a fast `success`. `/ai-approved`/`/ci-approved`/`/i18n-approved` (via `pr-commands.yml`, `issue_comment`-triggered) all check the commenter against **`.github/OWNERS`** (`no_parent_owners: true`, kept in sync with `.cursor/OWNERS`) at the PR's _base_ ref, never the PR's own branch -- so a PR can't self-approve by editing OWNERS in its own diff. A new commit to a sensitive path clears the "reviewed" label and re-blocks, same as `ai-config-reviewed` always did.

`pr_validation.yml` runs Jira, AI-config, CI-scripts, and i18n validation from a single `pr-validation` job/script (`src/validation/pr-validation/index.ts`) -- one checkout, one PR-files fetch shared between the path validations, and each check still isolated from the others' failures. The status contexts (`jira-validation`, `ai-config-validation`, `ci-scripts-validation`, `i18n-validation`), their labels, and the independent `/recheck-jira`/`/ai-approved`/`/ci-approved`/`/i18n-approved` commands stay independent.

**After this lands on `main`, add `i18n-validation` to branch protection's required status checks** (same chicken/egg as when `ci-scripts-validation` was introduced: `pull_request_target` reads the workflow from the base branch, so the status can't exist until the PR merges).
...
- **`ai-config-validation`/`ci-scripts-validation` publish a commit status only, not also a check-run.** `reportCommitStatus` (`label-sync.ts`) used to also call `checks.create()`/`checks.update()`, same mistake as `Merge Gate` above: an API-created check-run isn't attached to the `pr_validation.yml` run, so GitHub parks it under whatever check suite exists for the SHA (often "Needs Rebase"). Unlike `Merge Gate`, naming a job after the check wasn't an option -- `pr_validation.yml` runs Jira/AI-config/CI-scripts from one shared job, so no single job name can stand in for three distinct results. Fixed by dropping the check-run publish entirely; the commit status alone already satisfies `required_status_checks`, same as `jira-validation` (which never published a check-run) already showed.
