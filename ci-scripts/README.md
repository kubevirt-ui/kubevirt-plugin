...
The one exception to "neutral leaves the label untouched" is `retest-stale-gating.yml`'s own `mark-stale` job: when it flips `Run Gating Tests` to the stale `neutral` marker for a PR (`main` advanced past its last completed result), it also clears both `e2e-passed`/`e2e-failed` in the same step, on the same `should_mark` condition -- otherwise the PR list could keep showing "e2e-passed" for a result the required check itself now says is stale and unverified against the new base tip. This applies to every open PR the stale-marking touches, not just merge-pool PRs; a merge-pool PR gets a fresh real result soon after from the dispatched retest, while a non-pool PR simply has no e2e label until a push, `/retest-e2e`, or pool-entry supplies one.
