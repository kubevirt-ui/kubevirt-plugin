...
- **`/retest-e2e` must be able to find a `pr_number` dispatch's own run.** A `pr_number` retest (any of the dispatchers above) always runs against `ref: main`, so it carries no PR association, and its head SHA/branch are `main`'s, not the PR's -- none of the normal PR/SHA/branch match signals apply. `/retest-e2e` matches these via the `PR#<number> retest` token in `run-name:` instead, so a previous retest dispatch is re-run rather than silently re-dispatched from scratch every time.
