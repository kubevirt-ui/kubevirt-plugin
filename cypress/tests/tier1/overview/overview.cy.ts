...
describe('Test Virtualization Overview page', () => {
  ...
  // it's flaky
  xit('Verify links in get started are not outdated', () => {
  ...
  // it's flaky
  xit('ID(CNV-7927) Quick Starts card is displayed', () => {
  ...
  adminOnlyIT('ID(CNV-) click view all link on alerts card', () => {
    ...
    //todo: revert after https://issues.redhat.com/browse/CNV-51386
  ...
  adminOnlyIT('ID(CNV-) click the link on VM status', () => {
    ...
    cy.get('div.vm-statuses-card__body')
      ...
      .find('a')
      .click();
    //cy.contains(TEST_VM_NAME).should('exist');
  });
});
