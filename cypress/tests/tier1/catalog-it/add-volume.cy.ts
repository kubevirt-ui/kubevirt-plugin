...
describe('Test instanceType add volume modal', () => {
  xdescribe('Test add volume in Catalog', () => {
    before(() => {
      cy.visit('');
      cleanUp();
    ...
    it('ID(CNV-10530) add volume via registry', () => {
      cy.byButtonText(iView.addBtnText).click();
      addVolume(VOL_REGISTRY);
      cy.contains(VOL_REGISTRY.name, { timeout: 180000 }).should('exist');
      cy.deleteResources(K8S_KIND.DS, [VOL_REGISTRY.name], TEST_NS);
      cy.deleteResources(K8S_KIND.DV, [VOL_REGISTRY.name], TEST_NS);
      cy.deleteResources(K8S_KIND.PVC, [VOL_REGISTRY.name], TEST_NS);
      cy.wait(10000);
    });

    it('ID(CNV-10531) add volume via snapshot', () => {
      cy.byButtonText(iView.addBtnText).click();
      addVolume(VOL_VOLUME_SNAPSHOT);
      cy.contains(VOL_VOLUME_SNAPSHOT.name, { timeout: 10000 }).should('exist');
    });

    it('ID(CNV-9766) add volume via existing volume', () => {
      cy.byButtonText(iView.addBtnText).click();
      addVolume(VOL_VOLUME);
      cy.contains(VOL_VOLUME.name, { timeout: 180000 }).should('exist');
    });

    it('ID(CNV-9768) create VM from added volume with existing secret', () => {
  ...
  describe('Test add volume in Bootable volumes list', () => {
    ...
    it('ID(CNV-9766) add volume via upload in Bootable volumes list', () => {
    ...
    it('ID(CNV-9790) add volume via existing volume in Bootable volumes list', () => {
      cy.get('[data-test="item-create"]').click();
      cy.byButtonText(iView.withForm).click();
      addVolume(VOL_VOLUME);
      cy.contains(VOL_VOLUME.name, { timeout: 180000 }).should('exist');
