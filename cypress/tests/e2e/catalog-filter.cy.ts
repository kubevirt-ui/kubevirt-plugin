...
describe('Test filter in Templates', () => {
  ...
  it('filter VM catalog by text', () => {
    cy.get(cView.filterText).type('centos-stream9-server-small');
    cy.get(cView.GRID).find(cardTitleV6).should('contain', 'CentOS Stream');
    ...
    cy.get(cView.GRID).find(cardTitleV6).should('not.contain', 'Fedora');
  ...
  it('filter VM catalog by template boot source', () => {
    ...
    cy.get(cView.GRID).find(cardTitleV6).should('contain', 'CentOS Stream 10');
