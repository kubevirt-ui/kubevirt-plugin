...
describe('Create VMs from InstanceType', () => {
  ...
  // disabled until "Create VirtualMachine" button will not miss the click
  ...
  it('verify customized VM from IT', () => {
    cy.contains(descrText, 'u1.small').should('be.visible');
    cy.contains(descrText, 'rhel.9').should('be.visible');
    ...
    cy.contains(VM_IT_CUST.bootMode).should('be.visible');
