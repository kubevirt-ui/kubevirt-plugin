...
adminOnlyDescribe('Test template clone edit template data', () => {
  ...
  describe('Create VM from the cloned template', () => {
    it('ID(CNV-9546) create VM from the cloned template', () => {
      ...
      vm.create(vmFromCloneTemplate, false);
    ...
    xit('ID(CNV-9546) verify VM disks', () => {
      tab.navigateToConfigurationStorage();
      cy.contains('[data-test-rows="resource-row"]', 'disk-blank').should('exist');
      cy.contains('[data-test-rows="resource-row"]', 'disk-url').should('exist');
      cy.contains('[data-test-rows="resource-row"]', 'disk-container').should('exist');
    });

    xit('ID(CNV-9546) verify VM scripts details', () => {
      vm.delete(vmFromCloneTemplate.name);
      cy.deleteResource(K8S_KIND.Secret, cloneTemplate.newSecret, 'openshift');
      cy.deleteResource(K8S_KIND.Secret, cloneTemplate.newSecret, TEST_NS);
    });
  });

  describe('Add nic to the cloned template', () => {
    // https://issues.redhat.com/browse/CNV-55909
    xit('ID(CNV-1850) add 2nd nic to the cloned template', () => {
      cy.visitTemplates();
      cy.switchProject(ALL_PROJ_NS);
    ...
    // TODO: https://issues.redhat.com/browse/CNV-50292
    xit('ID(CNV-9791) create VM from the template and verify the 2nd nic is attached', () => {
      cy.visitCatalog();
      cy.switchProject(TEST_NS);
