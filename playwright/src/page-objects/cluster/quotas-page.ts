// QuotasPage — Page object for quotas interactions.
...
export default class QuotasPage extends BasePage {
  ...
  async verifyEmptyStateVisible(): Promise<boolean> {
    try {
      const emptyHeading = this.locator('h4:has-text("No application-aware quotas found")');
      await emptyHeading.waitFor({ state: 'visible', timeout: TestTimeouts.DEFAULT });
      return true;
    } catch {
      return false;
    }
  ...
  async verifyEmptyStateGuidanceVisible(): Promise<boolean> {
    try {
      const guidance = this.locator(
        'text=Click Create quota to create your first application-aware quota',
      );
      await guidance.waitFor({ state: 'visible', timeout: TestTimeouts.SHORT_WAIT });
      return true;
    } catch {
      return false;
    }
  ...
  async getDocumentationLinkHref(): Promise<null | string> {
    try {
