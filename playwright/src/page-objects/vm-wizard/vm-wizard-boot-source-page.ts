// VmWizardBootSourcePage — Page object for vm wizard boot source interactions.
...
export default class VmWizardBootSourcePage extends PageCommons {
  ...
  async getFirstVolumeName(): Promise<string> {
    try {
      const firstRow = this._pfV6CWizardTableTbodyTr.first();
      await firstRow.waitFor({ state: 'visible', timeout: TestTimeouts.SHORT_WAIT });
