  ...
  () => {
    test('Description and boot order edits persist across Back/Next navigation', async ({
      ...
      await test.step('Step 3: Boot source — select the first available volume', async () => {
        ...
        await expect
          .poll(() => vmWizardBootSourcePage.getBootVolumeCount(), {
            message: 'At least one boot volume is required to exercise boot-order reordering',
            timeout: utils.TestTimeouts.UI_ELEMENT_VISIBILITY,
          })
          .toBeGreaterThan(0);
      ...
      await test.step('Step 6: Review and create — verify prior selections are displayed', async () => {
        ...
        // Custom configuration is a non-clone flow: the Review step's Name field is static
