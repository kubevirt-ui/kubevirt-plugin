...
import { createExampleVM, deleteResource, ocIgnore } from '../../utils/oc';
...
test.describe('Check all virtualization pages can be loaded', () => {
  test.afterAll(() => {
    if (!NS) return;
    deleteResource('vm', DEFAULT_VM_NAME, NS);
  });

  test.beforeAll(() => {
    if (!NS) return;
    createExampleVM(DEFAULT_VM_NAME, NS);
    for (const cmd of [
  ...
  test('start example vm', async ({ page, vmDetails, vmList }) => {
    ...
    if (await dialog.isVisible({ timeout: 3 * SECOND }).catch(() => false)) {
  ...
  test('check the status of example vm', async ({ page, vmDetails, vmList }) => {
    await vmList.navigateToVMList(NS);
    expect(
      await vmList.waitForRow(DEFAULT_VM_NAME, 30 * SECOND),
      `VM "${DEFAULT_VM_NAME}" not found in namespace "${NS}"`,
    ).toBe(true);
    await vmList.openVM(DEFAULT_VM_NAME);
    // Wait for the VM details page to load before switching tabs
    await expect(page.locator('h1').getByText(DEFAULT_VM_NAME)).toBeVisible({
      timeout: 30 * SECOND,
    });
    await vmDetails.goToTab('Overview');
    await vmDetails.expectOverviewStatus('Running');
  });
  ...
  test.skip('Other resources list page tests are skipped', () => {
