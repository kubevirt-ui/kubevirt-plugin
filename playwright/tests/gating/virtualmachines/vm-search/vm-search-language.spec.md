...
- **Objective:** Verify that when a submitted query matches zero VMs, the "No results found for ..."
  empty state renders every token using search-language syntax (`key:value`, exclusions prefixed with
  `-`) instead of the pre-fix raw filter dump (e.g. `os:!RHEL` or comma-joined values).
- **Target version:** CNV 5.0.0
...
| 2    | Observe the filtered empty state | Text equals `No results found for status:Paused -os:RHEL`                         |
